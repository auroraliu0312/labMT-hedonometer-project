...
# Use scored dataframe
df_dict = df_scored.copy()

# Define core columns for analysis
core_columns = [
    'object_id', 'title', 'category', 'department', 
    'culture', 'period', 'object_date', 'century',
    'score', 'matched', 'total', 'coverage'
]

# Keep only columns that exist
available_cols = df_dict.columns.tolist()
dict_columns = [col for col in core_columns if col in available_cols]

print(f"Creating dictionary for {len(dict_columns)} columns")
...
for col in dict_columns:
    missing_count = df_dict[col].isna().sum()
...
for col in dict_columns:
    # Get data type
    col_dtype = str(df_dict[col].dtype)
    
    # Get counts
    non_null = df_dict[col].count()
    missing_count = missing_stats[col]['count']
    missing_pct = missing_stats[col]['percentage']
    
    # Get example value
    if non_null > 0:
        example = df_dict[col].dropna().iloc[0]
    ...
    # Column descriptions
    descriptions = {
        'object_id': 'Unique Met Museum object identifier',
        'title': 'Artwork title (raw, as provided by API)',
        'category': 'Cultural category: "eastern" or "western"',
        'department': 'Met curatorial department',
        'culture': 'Cultural attribution (e.g., "Japanese", "French")',
        'period': 'Historical period (e.g., "Edo period")',
        'object_date': 'Object date description (e.g., "ca. 1880")',
        'century': 'Century derived from object_begin',
        'score': 'Happiness score (1-9 scale) from labMT',
        'matched': 'Number of words matched to labMT dictionary',
        'total': 'Total words in cleaned title',
        'coverage': 'Proportion of words matched (matched / total)'
    }
...
print(f"\n aved data dictionary to: {dict_path}")
...
# Save simple version
simple_dict = data_dict_df[['column', 'type', 'missing', 'missing_pct', 'description']].copy()
simple_path = TABLES_DIR / "met_data_dictionary_simple.csv"
simple_dict.to_csv(simple_path, index=False)
print(f"\n Saved simple data dictionary to: {simple_path}")

print("\n" + "=" * 70)
print("ALL PROCESSING COMPLETE")
print("=" * 70)
print("\nFiles created:")
print(f"  - Scored data: {PROCESSED_DIR / 'met_aesthetic_scored.csv'}")
print(f"  - Summary stats: {TABLES_DIR / 'met_aesthetic_summary.csv'}")
print(f"  - OOV analysis: {TABLES_DIR / 'met_aesthetic_oov.csv'}")
print(f"  - Data dictionary: {PROCESSED_DIR / 'met_data_dictionary.csv'}")
print(f"  - Simple dictionary: {TABLES_DIR / 'met_data_dictionary_simple.csv'}")
